(define (domain bakingrealistic)
	...
	(:action bake-cake
		...
		:precondition (and (set-oven-with-cake-bake-time-and-press-start ?oven ?cake ?mixture)
			...
			(mixture-has-blended-raw-egg-whites ?mixture)
		...
		:effect (and
			...
			(not (mixture-has-blended-raw-egg-whites ?mixture))
	...
	(:action bake-souffle
		...
		:precondition (and (set-oven-with-souffle-bake-time-and-press-start ?oven ?souffle ?mixture)
			...
			(container-in-oven ?pan ?oven)
			(not (mixture-is-hypothetical ?mixture))
			(mixture-has-folded-raw-egg-whites ?mixture)
			(mixture-has-raw-egg-yolk ?mixture)
			(mixture-has-butter ?mixture)
			(mixture-has-sugar ?mixture)
			(not (mixture-has-cups-of-flour ?mixture))
		...
		:effect (and
			(not (is-mixture ?mixture))
			(not (mixture-has-folded-raw-egg-whites ?mixture))
			(not (mixture-has-raw-egg-yolk ?mixture))
			(not (mixture-has-butter ?mixture))
			(not (mixture-has-sugar ?mixture))
	...
	(:action fold-egg-whites-into-mixture
		...
		:effect (and
			...
			(not (is-egg ?e))
			(mixture-has-folded-raw-egg-whites ?m)
			(not (egg-in-container ?from ?e)))
	)

	(:action place-dessert-on-plate
		:parameters (?from - container ?to - container ?dessert - dessert_hypothetical)
		:precondition (and (move-baked-good-in-container-to-different-container ?from ?to ?dessert)
			(dessert-in-container ?from ?dessert)
			(not (dessert-is-hypothetical ?dessert))
			(is-plate ?to)
			(not (container-in-an-oven ?to))
			(not (container-in-an-oven ?from))
			(different ?from ?to))
		:effect (and
			(dessert-in-container ?to ?dessert)
			(not (pan-is-full ?from))
			(not (dessert-in-container ?from ?dessert)))
	)
